...
const Cart = () => {
  ...
  return (
    <div className=" w-[100vw] bg-[rgba(0,0,0,0.5)] fixed right-0 top-0 z-[100] transition transition-all ease-in-out duration-1000 " ref={cartRef}>
        ...
        <div className="product-container mt-[10px]">
          {cartItems.length >= 1 && cartItems.map((item) => (
            <div className="product py-[20px] px-[5px]" key={item._id}>
              <img src={urlFor(item?.image[0])} className="cart-product-image w-[25%] h-[25%] " />
              <div className=" w-[200px]">
                ...
                <div className="flex-wrap mt-[30px]">
                  ...
                  <p className=" border-[1px] border-solid border-[#eee] p-[6px]">
                    <span className="minus text-[16px] py-[6px] px-[12px] pointer text-[#f02d34] border-r-[1px] border-solid border-[gray]" onClick={() => toggleCartItemQuanitity(item._id, 'dec') }>
                    <AiOutlineMinus />
                    </span>
                    <span className="num  text-[20px] py-[6px] px-[12px] pointer  border-r-[1px] border-solid border-[gray]" onClick="">{item.quantity}</span>
                    <span className="plus  text-[16px] py-[6px] px-[12px] pointer text-[rgb(49, 168, 49)]  border-r-[1px] border-solid border-[gray]" onClick={() => toggleCartItemQuanitity(item._id, 'inc') }><AiOutlinePlus /></span>
                  </p>
                  </div>
                  <button
                    type="button"
        ...
        {cartItems.length >= 1 && (
          <div className="cart-bottom absolute bottom-[12px] right-[5px] w-full py-[30px] px-[64px]">
            ...
            <div className="btn-container w-[400px] m-auto">
              <button type="button" className="w-full max-w-[400px] py-[10px] px-[12px] rounded-[15px] border-none text-[20px] mt-[40px] uppercase bg-[#f02d34] text-[#fff] pointer " onClick={handleCheckout}>
                Pay with Stripe
              </button>
            </div>
          </div>
